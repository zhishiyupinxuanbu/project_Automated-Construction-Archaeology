...
REPORT_PLAN_TEMPLATE_DIR = SMART_TEMPLATE_DIR / "plans"
...
def command_doctor(args: argparse.Namespace) -> int:
    ...
    required_paths = [
        ...
        ("最终计划输出目录", REPORT_DIR),
        ("计划模板目录", REPORT_PLAN_TEMPLATE_DIR),
        ...
        ("默认计划基准模板", find_template_file("智能计划生成基准模板_北京卓凡文博技术有限公司_无分区.docx")),
        ("填报表生成脚本", SCRIPT_DIR / "create_smart_report_input_form.py"),
        ("人工表生成脚本", SCRIPT_DIR / "create_manual_form_from_project.py"),
        ("智能计划生成脚本", SCRIPT_DIR / "fill_smart_template_from_form.py"),
    ...
    print("")
    print_plan_template_matrix()
    print("")
    print("## 判断")
    plan_rows = inventory_plan_template_matrix()
    missing_plan = [row for row in plan_rows if not row["plan_template"]]
    if missing_plan:
        print(f"- 计划基准模板缺口：{len(missing_plan)} 个。请补齐 assets/templates/plans。")
    else:
        print("- 计划基准模板矩阵已覆盖。")
    print("- 计划流程入口：`preflight -> prepare --source-confirmed -> build-form -> recommend --plan -> generate-plan`。")
    return 0
