...
Mark `smart-prospecting-plan` as the current formal local delivery version for colleague installation and cross-computer migration. Preserve traceability without introducing Git as a required maintenance tool.
...
- Kept default final 计划 output on the user's Desktop.
...
- Keep final 计划 outputs on the user's Desktop by default.

### Validation To Run After Any Copy Or Update

```bash
python3 ~/.codex/skills/smart-prospecting-plan/scripts/self_test_skill.py
python3 ~/.codex/skills/smart-prospecting-plan/scripts/verify_manifest.py --strict
python3 ~/.codex/skills/smart-prospecting-plan/scripts/run_smart_report_workflow.py doctor
```
