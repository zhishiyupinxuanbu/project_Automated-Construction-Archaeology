...
def _has_attachment_component(items: Sequence[str], keywords: Sequence[str]) -> bool:
    return any(all(keyword in item for keyword in keywords) for item in items)


def complete_hecha_attachments(data: HechaRequestData) -> List[str]:
    """Keep the required attachment framework even when source materials are incomplete."""
    provided = [item for item in data.attachments if item]
    if not provided:
        return default_hecha_attachments(data)

    defaults = default_hecha_attachments(data)
    required_checks = [
        (defaults[0], (("核准",), ("批复",), ("备案",), ("复函",), ("选址",), ("审批",), ("立项",))),
        (defaults[1], (("营业执照",), ("统一社会信用代码",))),
        (defaults[2], (("身份证",),)),
        (defaults[3], (("坐标", "Excel"), ("经纬度", "Excel"))),
        (defaults[4], (("KML",),)),
        (defaults[5], (("宗地图",),)),
    ]
    ...
    for default_item, alternatives in required_checks:
        match_index = next(
            (
                index
                for index, item in enumerate(provided)
                if index not in used_indexes
                and any(_has_attachment_component([item], keywords) for keywords in alternatives)
            ),
            None,
        )
        if match_index is None:
            ...
            completed.append(provided[match_index])
            used_indexes.add(match_index)

    completed.extend(
        item for index, item in enumerate(provided)
        if index not in used_indexes and item not in completed
    )
    return completed
...
def create_hecha_docx(data: HechaRequestData, output_path: Path) -> Path:
    ...
    doc.add_paragraph()
    attachments = complete_hecha_attachments(data)
