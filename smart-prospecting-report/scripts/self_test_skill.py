#!/usr/bin/env python3
"""Validate the installed smart-prospecting-report skill package."""
...
SKILL_NAME = "smart-prospecting-report"
...
def main() -> int:
    ...
    if not skill_md.exists():
        ...
        if "我要写报告" not in text or "生成报告" not in text:
            fail("SKILL.md description lacks expected Chinese report triggers", errors)
    ...
    report_templates = list((skill_root / "assets/templates/reports").glob("*.docx"))
    if not report_templates:
        fail("no report templates found", errors)
    plan_template_dir = skill_root / "assets/templates/plans"
    if plan_template_dir.exists() and list(plan_template_dir.glob("*.docx")):
        fail("plan templates must not be bundled in the report skill", errors)
    ...
    if errors:
        ...
        for item in errors:
            print(f"- {item}")
        return 1
    print("OK smart-prospecting-report skill package")
    return 0
