...
class SkillPackageLayoutTest(unittest.TestCase):
    ...
    def test_reference_library_maintenance_scripts_are_external_data_aware(self) -> None:
        script_root = SKILL_DIR / "scripts" / "reference_library"
        scripts = sorted(script_root.rglob("*.py")) if script_root.exists() else []
        self.assertGreaterEqual(len(scripts), 7)
        missing_env_route = [
            str(path.relative_to(SKILL_DIR))
            for path in scripts
            if "HERITAGE_REFERENCE_LIBRARY" not in path.read_text(encoding="utf-8")
        ]
        self.assertEqual(missing_env_route, [])
